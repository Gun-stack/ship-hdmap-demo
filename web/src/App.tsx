...
export default function App() {
  ...
  // matters most, a page load that restores cam === "fly" before Unity has mounted its canvas.
  // A callback ref (forwardRef passes it straight to Unity's own canvas) makes that arrival a
  // state change Shortcuts.tsx can react to.
  ...
  return (
    <div className="app">
      ...
      {/* tabIndex=-1, not 0: the canvas must be focusable so Unity gets the flight keys, but it
          ...
          first place; the canvas still gets focus by a click here, or from Shortcuts.tsx's
          effect that focuses it whenever cam becomes "fly" -- by C, the toolbar button, or a
          reload that restores "fly" from persisted state. */}
      ...
      <PlanDock />
      <Shortcuts send={send} canvas={canvas} />

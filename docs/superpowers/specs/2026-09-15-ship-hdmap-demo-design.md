...
- 진입 불가 셀은 구획이 아니다: 시나리오 플래너의 접근 경로(이탈점 → 45° 사선 → `FINAL_RUN_M` 직진)를 그대로 놓아 보고, 이탈점이 차로 밖으로 나가거나 차체 스윕이 장애물과 겹치면 버린다. 차 한 대 들어갈 수 없는 자리에 주차면을 그리지 않는 실제 주차장 설계 규칙과 같다. `FINAL_RUN_M` 은 Unity `ScenarioPlanner.FinalRunM` 과 같은 값이어야 한다(5.0 m)
- 선수→선미, 깊은 곳 먼저 순으로 `sequence_no`. 이 순서는 기능이다 — 모든 접근 경로가 목표의 선미 쪽에서 이뤄지므로 선수부터 채우면 이미 주차된 차 위를 지나갈 일이 없다
- 격자 채우기이며 최적화 없음. 코드에 `ponytail:` 표시(업그레이드 경로: 2D bin packing)
- 이 규칙 아래 Deck 3 는 격자 136 셀 중 76 구획(활용률 23.4 %)이 남는다. 나머지는 중앙 차로 하나로 갑판 폭 전체를 덮는 배치의 결과다 — 활용률을 되찾으려면 통로(aisle) 를 가진 갑판 배치가 필요하고, 이는 M6 이후 과제
...
| M5d 측위 상실·회복 | `Localizer` 가 달성 σ 를 함께 내고, 커버리지 예측과 대조해 저하·상실을 판정하고, 지나온 자취를 되짚어 회복하고, 못 나오면 구획을 `unreachable` 로 포기한다 (`2026-09-17-m5d-localization-loss-design.md`) | 바깥 열 구획을 목표로 하면 예측한 지점에서 상실 → 역추적 → 회복 또는 스킵이 일어난다 |

...
namespace ShipHdMap
{
    ...
    public static class MapOverlay
    {
        ...
        public static GameObject Build(VehicleMap map, Transform parent)
        {
            ...
            foreach (var slot in map.parking_slots ?? new List<ParkingSlot>()) Line(root, slot.deck_id, slot.id, slot.polygon, SlotColor, SlotWidth);
            return root;
        }
        ...
        static void Line(GameObject root, string deckId, string id, double[][] pts, Color c, float width)
        {
